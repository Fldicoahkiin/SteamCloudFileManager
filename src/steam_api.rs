...
impl SteamCloudManager {
    ...
    fn cleanup_app_id_file() {
        // 清理steam_appid.txt 文件
        if std::path::Path::new("steam_appid.txt").exists() {
    ...
    pub fn connect(&mut self, app_id: u32) -> Result<()> {
        ...
        // 先尝试使用环境变量初始化
        let mut init_result = Client::init();
        
        if init_result.is_err() {
            // 环境变量方式失败，创建 steam_appid.txt 文件
        ...
        let (client, single) = init_result
            .map_err(|e| anyhow!("无法初始化Steam API: {:?}", e))?;
    ...
    pub fn is_cloud_enabled_for_account(&self) -> Result<bool> {
        ...
        let client = client.as_ref().ok_or_else(|| anyhow!("Steam客户端未连接"))?;
    ...
    pub fn get_files(&self) -> Result<Vec<CloudFile>> {
        ...
        for steam_file in steam_files {
            ...
            let timestamp = Local.timestamp_opt(steam_file_handle.timestamp(), 0)
                .single()
